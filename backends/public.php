...
// 		echo $REFERRER;
		...
		$subdomain = $host[0];
		$categoryId = $this->model->getCategoryIdByUrl($subdomain);
...
// 		Info of the Application
		...
		switch ($section) 
		{
			case 'mainSection':
				...
				$data['categoryInfo'] = $this->model->getCategoryInfoById($categoryId);
				
				//		get the category's subcategories
				$data['subcategories'] = $this->model->getSubcategoriesByCategoryId($categoryId);
				
				//		get companies by category
				$data['companies'] = $this->model->getCompaniesByCategoryId($categoryId);
				$i = 0;
				foreach ($data['companies'] as $company)
				{
					 $subcategories = $this->model->getSubcategoriesByCompanyId($company['company_id']);
					 $data['companies'][$i]['subcategories'] = $subcategories;
					 $i++;
				}
			break;
