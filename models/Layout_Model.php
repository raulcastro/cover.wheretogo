...
class Layout_Model
{
	...
	{
		try {
			$query = 'SELECT category_id FROM categories WHERE url = "'.$url.'"';
	...
	public function getPromotedCompaniesByCategory($categoryId)
	{
		try {
			$query = 'SELECT
					c.company_id,
					...
					LEFT JOIN categories cat ON c.category = cat.category_id
					LEFT JOIN company_logo cl ON cl.company_id = c.company_id
					WHERE c.promoted = 1 AND c.category = '.$categoryId;
...
// 			echo $query;
	...
	public function getSubcategoriesByCompanyId($companyId)
	{
		try {
			$query = 'SELECT s.`name` 
					FROM companies_subcategories cs
					LEFT JOIN subcategories s ON s.`subcategory_id` = cs.`subcategory`
					WHERE cs.company = '.$companyId;
			return $this->db->getArray($query);
		} catch (Exception $e) {
			return false;
		}
